...
const CaptainTracking = () => {
  ...
  console.log("CaptainTracking initial ride:", initialRide);
  ...
  // 1. Fetch existing path on refresh
  useEffect(() => {
    ...
    if (socket) {
      watchId.current = navigator.geolocation.watchPosition(
        (position) => {
          ...
          // Only extend the path if the ride has actually started
  ...
  return (
    <div className="h-screen w-screen relative bg-zinc-900 overflow-hidden">
      ...
      <div className='absolute inset-0 z-0 bg-slate-300 transition-all duration-300'>
        ...
        <div className="absolute inset-0 opacity-10 pointer-events-none z-10" style={{ backgroundImage: 'radial-gradient(#fff 1px, transparent 0)', backgroundSize: '30px 30px' }}></div>
      </div>

      {/* 2. Bottom Control Sheet */}
      <motion.div 
        initial={{ y: "100%" }}
        animate={{ y: isSheetHidden ? "calc(100% - 70px)" : 0 }}
        transition={{ type: "spring", damping: 25, stiffness: 120 }}
        className="absolute bottom-0 left-0 right-0 bg-white rounded-t-4xl p-6 z-20 shadow-2xl"
      >
        {/* Pull Bar */}
        <div 
        ...
        <div className={`${isSheetHidden ? 'opacity-0 pointer-events-none' : 'opacity-100'} transition-opacity duration-300`}>
            {/* Rider Details */}
            ...
            <div className="space-y-4">
              <AnimatePresence mode="wait">
                ...
                {rideStatus === 'ARRIVED' && (
                  <motion.div key="otp-panel" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-4">
